  ...
  try {
    ...
    const invoiceResponse = await fetch("https://api.nowpayments.io/v1/invoice", {
      ...
      body: JSON.stringify({
        ...
        success_url: `${Deno.env.get("SITE_URL") || supabaseUrl.replace(".supabase.co", ".lovable.app")}/dashboard/wallet?status=success`,
        cancel_url: `${Deno.env.get("SITE_URL") || supabaseUrl.replace(".supabase.co", ".lovable.app")}/dashboard/wallet?status=cancelled`,
      }),
    });
